...
class Event():
    ...
    def __init__(self, g1, g2, inclusion, inversed_inclusion, inclusions):
        ...
        self.events = {1:'merging',
                       ...
                       7:'forming',
                       0:'no_event'}
        # for merge & split ----------- ####### needs correction later
        self.results = []
    ...
    def is_dissolving(self,):
        ...
        # print("cid", self.g2.graph['cid'].split('_'), self.g2.graph['cid'].split('_')[1], len(comm))
        if not (len(comm) > 1 and self.g2.graph['cid'].split('_')[1] == comm[-1]):
            return False
    ...
    def is_forming(self,):
        ...
        if not (len(comm) > 1 and self.g1.graph['cid'].split('_')[1] == comm[-1]):                                                #to avoid multiple formation of the same community
            return False
